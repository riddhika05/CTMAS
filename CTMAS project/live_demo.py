...
def run_live_demo(model, explainer, normal_df, attack_df):
    clear_screen()
    print("=" * 70)
    ...
    try:
        while True:
            ...
            if pred == 0:
                ...
                print(f"[{timestamp}] {YELLOW}XAI Engine analyzing attack vector...{RESET}")
                ...
                explanation = f"Attack detected due to abnormal behavior in {', '.join(top_feats)}"
                
                print(f"[{timestamp}] {BOLD}XAI Explanation:{RESET} {explanation}")
                print(f"[{timestamp}] {BOLD}Sensor Details:{RESET}")
...
if __name__ == "__main__":
    ...
    if model is not None:
        time.sleep(1)
        run_live_demo(model, explainer, normal_df, attack_df)
